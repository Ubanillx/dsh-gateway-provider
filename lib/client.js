...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var T = ZH ? {
			...
			imageInput: "图片输入", imageAuto: function (img) { return img ? "自动（models.dev：支持图片）" : "自动（按 models.dev 目录）"; }, imageYes: "支持图片", imageNo: "仅文本",
			imageHint: "留空自动判定；未收录于 models.dev 的模型按仅文本处理",
			protocolAuto: function (p) { return p ? "自动（" + p + "）" : "自动"; },
			discoveredAs: function (v) { return "发现值：" + v; },
			overriding: function (v) { return "覆盖中 · 发现值：" + v; },
			outLabel: "输出",
			deleted: "✓ 已删除",
			...
			imageInput: "Image input", imageAuto: function (img) { return img ? "Auto (models.dev: image-capable)" : "Auto (follow models.dev)"; }, imageYes: "Image-capable", imageNo: "Text only",
			...
			overriding: function (v) { return "Overridden · discovered: " + v; },
			outLabel: "out",
			deleted: "✓ Deleted",
		...
		/**
		 * The model-override fields the settings schema declares. Everything
		 * else on a rendered row is UI-local display state.
		 */
		var PERSISTED_MODEL_KEYS = ["id", "name", "protocol", "contextWindow", "maxTokens", "disabled", "reasoningLevels", "inputModalities"];
		/** The gateway fields the settings schema declares. */
		var PERSISTED_GATEWAY_KEYS = ["id", "label", "baseURL", "openaiURL", "responsesURL", "anthropicURL", "apiKeyEnv", "flavor", "catalogMode", "userId", "endpointPriority", "models"];

		/**
		 * Reduce one value to what may be persisted: `undefined` / `null` / `""`
		 * and empty arrays all mean "unset" for these fields. An explicitly
		 * `undefined` member is not cosmetic — the settings RPC arg codec is a
		 * JSON-value union with no `undefined`, so one such key rejects the whole
		 * write, and the rejection used to be invisible (console-only). An empty
		 * array is worse than useless: `[]` reads back as a set override in the
		 * UI, and `endpointPriority: []` silently disables the protocol order.
		 */
		...
		/** Copy only the schema-declared, actually-set fields of one object. */
		function persistableObject(source, keys) {
			var out = {};
			if (source === null || typeof source !== "object") return out;
			keys.forEach(function (k) {
				var v = source[k];
				if (definedValue(v)) out[k] = v;
			});
			return out;
		}

		/** One row stripped to its persistable override fields (no `_discovered*`). */
		function persistable(model) {
			return persistableObject(model, PERSISTED_MODEL_KEYS);
		}

		/** A whole model list stripped to persistable entries. */
		function persistableModels(list) {
			return (list || []).map(persistable);
		}

		/** A whole gateways array stripped to persistable entries. */
		function persistableGateways(list) {
			return (list || []).map(function (gw) {
				var out = persistableObject(gw, PERSISTED_GATEWAY_KEYS);
				if (Array.isArray(gw && gw.models)) out.models = persistableModels(gw.models);
				return out;
			});
		}
		...
		function mergeDiscovered(current, discovered) {
			...
			var updated = discovered.map(function (d) {
				...
				// Build the entry key by key: a `_discovered*` key holding
				// `undefined` would be copied into every write by the row editor
				// and rejected by the settings RPC codec.
				var entry = { id: d.id, _reasoning: d.reasoning === true };
				entry._discoveredName = definedValue(d.name) ? d.name : d.id;
				if (definedValue(d.protocol)) entry._protocol = d.protocol;
				if (definedValue(d.contextWindow)) entry._discoveredContext = d.contextWindow;
				if (definedValue(d.maxTokens)) entry._discoveredMax = d.maxTokens;
				if (typeof d.image === "boolean") entry._discoveredImage = d.image;
				if (prev !== undefined) {
					...
					["protocol", "contextWindow", "maxTokens", "reasoningLevels", "inputModalities", "disabled"].forEach(function (k) {
						// `[]` counts as unset here so a stray empty array from an
						// older write cannot survive a refresh as a fake override.
						if (definedValue(prev[k])) entry[k] = prev[k];
					});
				}
				return entry;
			});
		...
		function ModelEditor(props) {
			...
			var _v = React.useState({
				...
				// A stored empty array means "unset", not "text only": otherwise
				// merely opening and saving the row would pin the model to text.
				inputMode: Array.isArray(m.inputModalities) && m.inputModalities.length > 0
					? (m.inputModalities.indexOf("image") !== -1 ? "image" : "text")
					: "",
			...
			/** Hint for a capacity field: the row's own override, or the discovery value. */
			var capacityHint = function (typed, discovered) {
				if (!discovered) return null;
				return typed.length > 0 ? T.overriding(fmtTokens(discovered)) : T.discoveredAs(fmtTokens(discovered));
			};

			var submit = function () {
				if (!canSave) return;
				// Start from the persistable fields only: the row carries UI-local
				// `_discovered*` keys for display and the settings RPC arg codec
				// rejects an undefined-valued member outright.
				var next = persistable(m);
			...
			return h("form", { className: "na_editor", onSubmit: function (e) { e.preventDefault(); submit(); } },
				h("div", { className: "na_editorGrid" },
					...
					h(Field, { label: T.contextWindow, error: errors.contextWindow, hint: capacityHint(values.contextWindow, m._discoveredContext) },
					...
					h(Field, { label: T.maxTokens, error: errors.maxTokens, hint: capacityHint(values.maxTokens, m._discoveredMax) },
					...
					h(Field, { label: T.imageInput, hint: T.imageHint },
						h("select", { className: "na_input", value: values.inputMode, onChange: function (e) { set("inputMode", e.target.value); } },
							h("option", { value: "" }, T.imageAuto(m._discoveredImage === true)),
		...
		function ModelRow(props) {
			...
			var hasLevels = Array.isArray(m.reasoningLevels) && m.reasoningLevels.length > 0;
			var hasModalities = Array.isArray(m.inputModalities) && m.inputModalities.length > 0;
			// The row shows the EFFECTIVE values (override first, discovery second),
			// so a hand-filled parameter is visible right where it is edited — the
			// discovered numbers alone made a successful override look like nothing
			// had been filled in.
			var contextWindow = m.contextWindow != null ? m.contextWindow : m._discoveredContext;
			var maxTokens = m.maxTokens != null ? m.maxTokens : m._discoveredMax;
			var protocol = m.protocol || m._protocol;
			var metaParts = [];
			if (protocol) metaParts.push(protocol);
			if (contextWindow) metaParts.push(fmtTokens(contextWindow) + " ctx");
			if (maxTokens) metaParts.push(fmtTokens(maxTokens) + " " + T.outLabel);
			if (m._reasoning === true || hasLevels) metaParts.push("reasoning");
			if (m._discoveredImage === true || (hasModalities && m.inputModalities.indexOf("image") !== -1)) metaParts.push("image");
			// `[]` is truthy in JS but means "unset" for these list fields.
			var overridden = m.protocol || m.contextWindow || m.maxTokens || hasLevels || hasModalities
				|| (m.name && m.name !== m._discoveredName);
		...
		function ModelList(props) {
			...
			return h("div", { className: "na_models" },
				h("div", { className: "na_toolbar" },
					...
					props.status ? h("span", { className: "na_badge " + (props.status.ok ? "na_testOk" : "na_testFail") }, props.status.label) : null,
				),
		...
		function GatewayModelsSection(props) {
			...
			// Per-gateway outcome of the last per-model write (save / hide / add /
			// delete). These writes used to be fire-and-forget, so a rejected write
			// looked exactly like a successful one.
			var _ms = React.useState({}); var modelStatus = _ms[0]; var setModelStatus = _ms[1];
			var statusTimers = React.useRef({});
			...
			/** Re-attach last-discovery meta to persisted (clean) entries for display. */
			var withMeta = function (list, cacheKey) {
				var meta = metaCache[cacheKey] || [];
				return (list || []).map(function (e) {
					var mHit = null;
					for (var i = 0; i < meta.length; i++) if (meta[i].id === e.id) { mHit = meta[i]; break; }
					if (mHit === null) return e;
					var out = Object.assign({}, e);
					// Never attach an undefined-valued key: these rows are handed
					// straight back to settings/mutate by the row editor, whose arg
					// codec rejects `undefined` and rejected the whole write.
					if (out._discoveredName === undefined && definedValue(mHit._discoveredName)) out._discoveredName = mHit._discoveredName;
					if (out._protocol === undefined && definedValue(mHit._protocol)) out._protocol = mHit._protocol;
					if (out._discoveredContext === undefined && definedValue(mHit._discoveredContext)) out._discoveredContext = mHit._discoveredContext;
					if (out._discoveredMax === undefined && definedValue(mHit._discoveredMax)) out._discoveredMax = mHit._discoveredMax;
					if (out._reasoning === undefined && typeof mHit._reasoning === "boolean") out._reasoning = mHit._reasoning;
					if (out._discoveredImage === undefined && typeof mHit._discoveredImage === "boolean") out._discoveredImage = mHit._discoveredImage;
					return out;
				});
			};
			var load = React.useCallback(function () {
				api.settings.describe({}).then(function (res) {
					...
					setSnapshot(Object.assign({}, snap, {
						models: withMeta(snap.models || [], "default"),
						gateways: (snap.gateways || []).map(function (g) { return Object.assign({}, g, { models: withMeta(g.models || [], "gateway:" + g.id) }); }),
					}));
			...
			/** Mutate one path op into the settings namespace; always reload
			  * afterwards (success resyncs the ref, failure rolls it back).
			  * Resolves true on success, or the rejection reason string. */
			var mutate = function (ops) {
				return api.settings.mutate({ ns: NS, ops: ops }).then(function (res) {
					if (!(res && res.result && res.result.ok)) {
						...
						console.warn("llm-newapi: settings write failed", raw);
						load();
						return msg || "settings write rejected";
					}
					load();
					return true;
				}).catch(function (err) {
					...
					load();
					var why = (err && err.message) || "settings write rejected";
					var cause = err && err.cause && err.cause.message;
					return cause ? why + " :: " + cause : why;
				});
			};
			...
			/** Human-readable tail of a failed write (`mutate` resolves the reason
			  * as a string; a synchronous throw arrives as an Error). */
			var reasonOf = function (written) {
				if (typeof written === "string" && written.length > 0) return " — " + written;
				var message = written && written.message ? String(written.message) : "";
				var cause = written && written.cause && written.cause.message ? String(written.cause.message) : "";
				var detail = message.length > 0 ? message : cause;
				return detail.length > 0 ? " — " + detail : "";
			};

			/** Run one write attempt, turning a synchronous throw (e.g. the RPC arg
			  * codec rejecting the payload) into a rejection the reporter can show. */
			var attemptWrite = function (run) {
				try { return Promise.resolve(run()); }
				catch (error) { return Promise.reject(error); }
			};

			/**
			 * Report the outcome of one per-model write next to the model list.
			 * `pending` is an `attemptWrite`/`mutate` result, which resolves `true`
			 * or carries the rejection reason.
			 */
			var reportModelWrite = function (key, pending, okLabel) {
				return pending.then(function (written) {
					var ok = written === true;
					setModelStatus(function (prev) {
						return Object.assign({}, prev, { [key]: { ok: ok, label: ok ? (okLabel || T.configSaved) : T.configSaveFail + reasonOf(written) } });
					});
					if (statusTimers.current[key] !== undefined) clearTimeout(statusTimers.current[key]);
					statusTimers.current[key] = setTimeout(function () {
						delete statusTimers.current[key];
						setModelStatus(function (prev) { var next = Object.assign({}, prev); delete next[key]; return next; });
					}, 6000);
					return written;
				});
			};
			...
			/** Write a model entry (default gateway → root `models`, else
			  ...
			  * back-to-back edits do not lose each other. Every entry — the edited
			  * row as well as the untouched ones — is reduced to the persistable
			  * override fields first: rows carry UI-local `_discovered*` keys and
			  * the resolved snapshot carries schema-materialized empty arrays. */
			var writeModels = function (gwIdx, transform) {
				var snap = snapRef.current || {};
				if (gwIdx === -1) {
					var nextModels = persistableModels(transform(snap.models || []));
					syncModels(nextModels);
					return mutate([{ op: "set", path: ["models"], value: nextModels }]);
				}
				var gws = snap.gateways || [];
				var nextGateways = gws.slice();
				nextGateways[gwIdx] = Object.assign({}, gws[gwIdx], { models: persistableModels(transform((gws[gwIdx] && gws[gwIdx].models) || [])) });
				nextGateways = persistableGateways(nextGateways);
			...
			var fetchModels = function (gwIdx) {
				...
				discover(gwIdx, gw.baseURL, provider).then(function (found) {
					...
					var discovered = found
						...
						.map(function (m) {
							var entry = { id: m.id };
							["name", "contextWindow", "maxTokens", "protocol", "reasoning"].forEach(function (k) {
								if (m[k] !== undefined && m[k] !== null) entry[k] = m[k];
							});
							return entry;
						});
					var metaList = mergeDiscovered(gwIdx === -1 ? (snap.models || []) : ((snap.gateways || [])[gwIdx] || {}).models || [], discovered);
					metaCache[gwIdx === -1 ? "default" : "gateway:" + ((snap.gateways || [])[gwIdx] || {}).id] = metaList;
					// Persist override fields only: the discovered-meta (_discovered*)
					// keys are UI-local display state, and an undefined-valued member
					// breaks the settings RPC arg codec (a JSON-value union).
					var cleanList = persistableModels(metaList);
					writeModels(gwIdx, function () { return cleanList; }).then(function (written) {
						if (gwIdx === -1) syncModels(metaList);
						else if (written === true) {
							var g = snapRef.current || {};
							var nextGws = (g.gateways || []).map(function (x, i) { return i === gwIdx ? Object.assign({}, x, { models: metaList }) : x; });
							syncGateways(nextGws);
						}
						if (written !== true) {
							setTestStatus(function (prev) { return Object.assign({}, prev, { [key]: { ok: false, label: T.configSaveFail + " \u2014 " + written } }); });
						}
					});
			...
			var saveGatewayConfig = function (gwIdx, changes) {
				...
				if (gwIdx === -1) {
					...
					if (touched) {
						...
						// Persist declared fields only: the resolved snapshot carries
						// schema-materialized `endpointPriority: []`, which reads back
						// as an explicit (empty) preference order.
						nextGateways = persistableGateways(nextGateways);
...
var fail = results.map(function (r) { return r && r.result && r.result.error && r.result.error.message; })
			...
			var addGateway = function (spec) {
				...
				var nextGateways = persistableGateways(gws.concat([entry]));
				syncGateways(nextGateways);
				var writes = [
...
var fail = results.map(function (r) { return r && r.result && r.result.error && r.result.error.message; })
			...
			var deleteGateway = function (gwIdx) {
				...
				var gws = currentGateways();
				// Read the id before the optimistic sync drops the entry.
				var key = String((gws[gwIdx] || {}).id || gwIdx);
				var nextGateways = persistableGateways(gws.filter(function (_, i) { return i !== gwIdx; }));
				syncGateways(nextGateways);
				reportModelWrite(key, attemptWrite(function () { return mutate([{ op: "set", path: ["gateways"], value: nextGateways }]); }), T.deleted);
			};
			...
			return h("div", { className: "na_section" },
				...
				h(GatewayCard, {
					...
					onModelChange: function (entry) { reportModelWrite("default", attemptWrite(function () { return writeModels(-1, function (arr) { return upsertModel(arr, entry); }); })); },
					onModelDelete: function (id) { reportModelWrite("default", attemptWrite(function () { return writeModels(-1, function (arr) { return arr.filter(function (m) { return m.id !== id; }); }); }), T.deleted); },
					onAddModel: function (entry) { reportModelWrite("default", attemptWrite(function () { return writeModels(-1, function (arr) { return upsertModel(arr, entry); }); })); },
				}),
				// Additional gateways
				gateways.map(function (gw, i) {
					return h(GatewayCard, {
						...
						onModelChange: function (entry) { reportModelWrite(gw.id, attemptWrite(function () { return writeModels(i, function (arr) { return upsertModel(arr, entry); }); })); },
						onModelDelete: function (id) { reportModelWrite(gw.id, attemptWrite(function () { return writeModels(i, function (arr) { return arr.filter(function (m) { return m.id !== id; }); }); }), T.deleted); },
						onAddModel: function (entry) { reportModelWrite(gw.id, attemptWrite(function () { return writeModels(i, function (arr) { return upsertModel(arr, entry); }); })); },
					});
				}),
				// Add gateway form
		...
		exports.persistableModels = persistableModels;
		exports.persistableGateways = persistableGateways;
