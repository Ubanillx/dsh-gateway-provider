...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function mergeDiscovered(current, discovered) {
			...
			var updated = discovered.map(function (d) {
				...
				var entry = {
					...
					...typeof d.image === "boolean" ? { _discoveredImage: d.image } : {},
				};
				if (prev !== undefined) {
		...
		function ModelRow(props) {
			...
			if (m._discoveredImage === true || (Array.isArray(m.inputModalities) && m.inputModalities.indexOf("image") !== -1)) metaParts.push("image");
		...
		function GatewayModelsSection(props) {
			...
			var withMeta = function (list, cacheKey) {
				...
				return (list || []).map(function (e) {
					...
					if (out._discoveredName === undefined && mHit._discoveredName !== undefined) out._discoveredName = mHit._discoveredName;
					if (out._protocol === undefined) out._protocol = mHit._protocol;
					if (out._discoveredContext === undefined) out._discoveredContext = mHit._discoveredContext;
					if (out._discoveredMax === undefined) out._discoveredMax = mHit._discoveredMax;
					if (out._reasoning === undefined) out._reasoning = mHit._reasoning;
					if (out._discoveredImage === undefined && mHit._discoveredImage !== undefined) out._discoveredImage = mHit._discoveredImage;
					return out;
				});
			};
...
return Array.isArray(list) ? list : ((list && list.models) || []);
			...
			var fetchModels = function (gwIdx) {
				...
				discover(gwIdx, gw.baseURL, provider).then(function (found) {
					...
					// keys are UI-local and their undefined-valued members break the
					// settings RPC arg codec (zod record rejects undefined values).
					var cleanList = metaList.map(function (m) {
						var out = {};
						["id", "name", "protocol", "contextWindow", "maxTokens", "disabled", "reasoningLevels", "inputModalities"].forEach(function (k) {
